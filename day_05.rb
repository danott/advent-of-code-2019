...
class OpcodeTest < Minitest::Test
  ...
  def test_examples
    EXAMPLES.each do |input_state, expected_state|
      computer = Computer.new(state: input_state)
      computer.run
  ...
  def test_part_one_find_diagnostic_code
    ...
    computer = Computer.new(state: puzzle_input, input: [1]).run
  ...
  def test_part_two_example
    ...
    input_less_than_eight = Computer.new(state: state, input: [7]).run
    input_exactly_eight = Computer.new(state: state, input: [8]).run
    input_greater_than_eight = Computer.new(state: state, input: [9]).run
  ...
  def test_part_two_find_diagnostic_code
    puzzle_input = File.read("./day_05.input.txt").split(",").map(&:to_i)
    computer = Computer.new(state: puzzle_input, input: [5]).run
